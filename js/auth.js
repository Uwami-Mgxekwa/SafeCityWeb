...
const cityCoordinates = {
  ...
  "port-elizabeth": { lat: -33.958, lng: 25.6056, region: "Eastern Cape" },
  ...
  nelspruit: { lat: -25.4753, lng: 30.97, region: "Mpumalanga" },
...
async function checkExistingSession() {
  ...
  const sessionToken = getDetailsFromLocalstorage("safecity-session");
...
async function handleRegister(e) {
  ...
  try {
    ...
    if (newUser) {
      ...
      showAlert(
        "success",
        `Account created successfully, ${newUser.first_name}! Please sign in with your new account.`,
      );
...
function storeUserSession(user) {
  try {
    storeDetailsToLocalstorage("safecity-user", user);
    storeDetailsToLocalstorage("safecity-session", Date.now().toString());
    storeDetailsToLocalstorage("safecity-user-city", user.city);
    storeDetailsToLocalstorage("safecity-authstate", authState);
...
function showAlert(type, message) {
  ...
  alertIcon.querySelector("i").className =
...
const themeManager = {
  ...
  init() {
    const savedTheme = storeDetailsToLocalstorage("safecity-theme") || "light";
