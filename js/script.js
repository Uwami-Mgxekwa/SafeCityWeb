...
function initializeSupabase() {
  if (typeof window.supabase !== "undefined") {
    ...
    const SUPABASE_KEY =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNpZnNjZXFhdWxoemhrdmxnc2xhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTU5NDQxMzMsImV4cCI6MjA3MTUyMDEzM30.ywt4tUbKXMemF1FjJwKh4td46RqayaK4wJKYRfEw3RQ";
...
function checkAuthenticationAndInitialize() {
  ...
  const rememberedUser = getDetailsFromLocalstorage("safecity-user");
  const userLocationData = getDetailsFromLocalstorage("safecity-user-location");
  ...
  try {
    ...
    console.log(
      "User location:",
      appState.userLocation?.displayName || "Unknown",
    );
    ...
    removeDetailsFromLocalstorage("safecity-user");
    removeDetailsFromLocalstorage("safecity-user-location");
    window.location.href = "auth.html";
    return false;
  }
}
...
function updateProfileSection() {
  ...
  const userReports = appState.reports.filter(
    (report) =>
      report.user_id === appState.user.id ||
      report.user_email === appState.user.email,
  );
  const totalUpvotes = userReports.reduce(
    (sum, report) => sum + (report.upvotes || 0),
    0,
  );
...
async function handleReportSubmission() {
  ...
  if (!location) {
    alert(
      "Location is required. Please wait for location to be detected or update manually.",
    );
    return;
  }
  ...
  try {
    const newReport = {
      ...
      description:
        ...
        `${formatReportTitle({ type: appState.selectedIssueType })} reported by ${appState.user.first_name}`,
...
async function saveReportWithBackup(newReport) {
  try {
    ...
    const existingReports = JSON.parse(
      localStorage.getItem("safecity-reports") || "[]",
    );
...
async function loadFromSupabase() {
  try {
    ...
    console.log(
      `Loaded ${processedReports.length} total reports from Supabase`,
    );
...
function loadSampleData() {
  ...
  const sampleLocation = appState.userLocation?.coordinates || {
    lat: -26.2041,
    lng: 28.0473,
  };
...
async function initializeApp() {
  ...
  console.log(
    "SafeCity app initialized successfully with",
...
function handleLogout(e) {
  ...
  if (confirm("Are you sure you want to logout?")) {
    ...
    removeDetailsFromLocalstorage("safecity-user");
    removeDetailsFromLocalstorage("safecity-session");
    removeDetailsFromLocalstorage("safecity-user-city");
    removeDetailsFromLocalstorage("safecity-user-location");
  ...
  if (confirmBtn && selectElement) {
    confirmBtn.addEventListener("click", async function () {
      ...
      try {
        ...
        storeDetailsToLocalstorage("safecity-user", appState.user);
        storeDetailsToLocalstorage(
          "safecity-user-location",
          appState.userLocation,
        );
...
function renderMapMarkers() {
  ...
  console.log(
    `Rendered ${filteredReports.length} markers for filter: ${appState.currentFilter}`,
  );
}
...
function calculateStats() {
  ...
  const resolvedReports = appState.reports.filter(
    (r) => r.status === "resolved",
  ).length;
...
const themeManager = {
  ...
  init() {
    let savedTheme = JSON.parse(getDetailsFromLocalstorage("safecity-theme"));
    if(!savedTheme){
       savedTheme = "light"
    }
  ...
  setTheme(theme) {
    ...
    storeDetailsToLocalstorage("safecity-theme", theme);
  ...
  applyDarkModeSpecificStyles() {
    ...
    if (mapPlaceholder) {
      mapPlaceholder.style.background =
        "linear-gradient(45deg, #3a3a3a, #2d2d2d)";
    }
  },
  ...
  applyLightModeSpecificStyles() {
    ...
    if (mapPlaceholder) {
      mapPlaceholder.style.background =
        "linear-gradient(45deg, #f0f2f5, #e1e8ed)";
    }
  },
};
...
window.addEventListener("beforeunload", function (e) {
  ...
  if (description && description.value.trim() && selectedIssueType) {
    ...
    storeDetailsToLocalstorage("safecity-unsaved-report", unsavedData);
  }
});
...
function loadUnsavedReport() {
  try {
    const unsavedData = getDetailsFromLocalstorage("safecity-unsaved-report");
    if (unsavedData) {
      ...
      removeDetailsFromLocalstorage("safecity-unsaved-report");
    }
  } catch (error) {
...
function checkForUpdates() {
  const lastVersion = getDetailsFromLocalstorage("safecity-version");
  ...
  storeDetailsToLocalstorage("safecity-version", APP_VERSION);
}

checkForUpdates();

console.log(
  "SafeCity JavaScript with enhanced authentication, user management, and city filtering initialized successfully",
);
